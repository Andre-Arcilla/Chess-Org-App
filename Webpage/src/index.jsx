...
const Overview = () => {
  // 1. Safely check if a user is already logged in via browser storage
  ...
  // Handle clicking a post card when on the public landing page
  const handleCardClick = () => {
    if (userData) {
      // If they are already logged in, send them to their respective dashboard
      window.location.href = userData.Role === 'Admin' ? '/dashboard-ADMIN.html' : '/dashboard-MEMBER.html';
    } else {
      // If public, tell them to log in, and focus the email input on the page if it exists
      alert("Please sign in to view full details.");
            ...
            <div 
              ...
              {extendedPosts.map((post, index) => (
                <div key={index} style={{
                  ...
                  {/* 3. Replaced React Router navigate() with native JS click handler */}
                  <div className="stat-item" onClick={handleCardClick} style={{
                    margin: '0 auto',
                    width: '100%',
                    height: '100%',
                    padding: '30px 50px',
...
// 4. Mount directly to the DOM without React Router
const container = document.getElementById('overview-root');
if (container) {
